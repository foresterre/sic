...
// todo: remove OperationId and str, and add Operation
...
/// Vec<&str> := Vector of unverified string arguments; initially with multiple arguments
...
#[derive(Debug, Clone)]
struct Op(Index, OperationId, Vec<String>);
...
//
// ```
// let mut tree: BTreeMap<Index, Op> = BTreeMap::new();
// op_by_index!(matches, "clap arg name", OperationId::Blur, tree)?;
// ```
macro_rules! op_by_index {
    ($matches:expr, $op_name:expr, $op_variant:expr, $container_map:expr) => {{
        ...
        let vec: Option<IndexedOps> = indices.and_then(|indices| {
            values.map(|values| {
                indices
                    .zip(values)
                    .map(|(i, v)| (i, Op(i, $op_variant, vec![v.to_string()])))
                    .collect::<_>()
            })
        });
        ...
        match last {
            Some(node) => {
                ...
                if (node.0 + 1) == current.0 {
                    ...
                    // with (index, op),
                    // `i"0", ndex` will be the index of the c"Unification of multi valued argument(s) failed:urrent node (i.e. de new last)
                    ...
                    let mut unified_values = unified_op.2;
                    unified_values.extend((current.1).2);
                    ...
                    //                  argument
                    // )
                    let updated_op = Op(current.0, unified_op.1, unified_values);
...
pub fn build_app_config<'a>(matches: &'a ArgMatches) -> Result<Config<'a>, String> {
    ...
    if let Some(script) = matches.value_of(ARG_APPLY_OPERATIONS) {
        ...
        let blur: Option<IndexedOps> = op_by_index!(matches, OP_BLUR, OperationId::Blur, tree);
        let _ = blur.map(|nodes| tree.extend(nodes));

        let brighten = op_by_index!(matches, OP_BRIGHTEN, OperationId::Brighten, tree);
        let _ = brighten.map(|nodes| tree.extend(nodes));

        let contrast = op_by_index!(matches, OP_CONTRAST, OperationId::Contrast, tree);
        let _ = contrast.map(|nodes| tree.extend(nodes));

        // crop, # of arguments = 4
        let crop = op_by_index!(matches, OP_CROP, OperationId::Crop, tree);
        tree_extend_unifiable(&mut tree, crop, 4)?;

        // crop, # of arguments = 9
        let filter3x3 = op_by_index!(matches, OP_FILTER3X3, OperationId::Filter3x3, tree);
        tree_extend_unifiable(&mut tree, filter3x3, 9)?;

        // TODO: if we have zero values, we need to do is_present (part of op_by_index)
        let fliph = op_by_index!(matches, OP_FLIP_HORIZONTAL, OperationId::FlipH, tree);
...
mod tests {
    ...
    mod cli_arg_values_unification {
        ...
        fn tree_extend_unifiable_n1() {
            ...
            let blur: IndexedOps = vec![(0, Op(0, OperationId::Blur, vec!["1".to_string()]))];
        ...
        fn tree_extend_unifiable_n4() {
            ...
            let blur: IndexedOps = vec![
                (0, Op(0, OperationId::Blur, vec!["1".to_string()])),
                (1, Op(0, OperationId::Blur, vec!["1".to_string()])),
                (2, Op(0, OperationId::Blur, vec!["1".to_string()])),
                (3, Op(0, OperationId::Blur, vec!["1".to_string()])),
            ];
        ...
        fn tree_extend_unifiable_n4_too_many_provided() {
            ...
            let blur: IndexedOps = vec![
                ...
                (2, Op(0, OperationId::Blur, vec!["1".to_string()])),
                (3, Op(0, OperationId::Blur, vec!["1".to_string()])),
                (4, Op(0, OperationId::Blur, vec!["1".to_string()])),
            ];
            let res = tree_extend_unifiable(&mut tree, Some(blur), 4);
